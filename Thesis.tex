...
\begin{document}
	...
	\begin{verbatim}
		...
		export JUPYTER_CONFIG_DIR=
				"/scratch.hpc/martin.tomassi/jupyter_config_for_job_$SLURM_JOB_ID"
		export MPLCONFIGDIR=
				"/scratch.hpc/martin.tomassi/matplotlib_cache_for_job_$SLURM_JOB_ID"
		mkdir -p "$JUPYTER_CONFIG_DIR"
		mkdir -p "$MPLCONFIGDIR"
		
		source venv_car/bin/activate
		jupyter nbconvert --to notebook 
				--execute car_emissions.ipynb 
				--output car_emissions_trained.ipynb
	...
	\subsection{Tecnologie e librerie}
	Il workflow sperimentale e di analisi è stato implementato in \texttt{Python}. Per le reti neurali (MLP e KAN) è stato utilizzato \textbf{PyTorch} (moduli \texttt{torch.nn}, \texttt{torch.optim}, \texttt{torch.nn.utils.prune} e utility custom per conteggio parametri e prunning). La famiglia degli ensemble è stata gestita con \textbf{scikit-learn} (Random Forest, pipeline e metriche) e con \textbf{XGBoost} per le sperimentazioni sul boosting e sul pruning cumulativo. Per l'elaborazione e la manipolazione dei dati si sono impiegati \texttt{pandas} e \texttt{numpy}; l'analisi statistica e il calcolo degli intervalli di confidenza (bootstrap) hanno fatto uso di \texttt{scipy.stats}. La visualizzazione è stata prodotta con \texttt{matplotlib} e \texttt{seaborn}. Tool ausiliari come \texttt{tqdm} sono stati adottati per le progress bar e \texttt{logging} per il tracciamento degli esperimenti; i risultati sperimentali sono stati serializzati in formato \texttt{JSON} con metadati (seed, configurazione, timestamp).
	
	\subsection{Ambienti di sviluppo e infrastruttura}
	La prototipazione ed il debug sono stati condotti su Google Colab tramite notebook interattivi; il training e le sperimentazioni su GPU sono stati eseguiti sul Cluster HPC dell'Università di Bologna con sottomissione batch tramite SLURM. Per la riproducibilità sono stati utilizzati ambienti virtuali (venv) ed un file requirements.txt per fissare le dipendenze. Ogni esperimento salva la propria configurazione in JSON (parametri, seed, metadati) e le esecuzioni su cluster producono output e notebook convertiti in HTML per la reportistica.
	
	\subsection{Linguaggi, scripting e automazione del workflow}
	Il linguaggio principale è Python; le analisi esplorative ed il reporting sono gestiti con Jupyter notebooks. La sottomissione batch e la gestione delle esecuzioni ripetute sono automatizzate con script bash e job SLURM; la conversione automatica ed esecuzione dei notebook viene eseguita tramite nbconvert. Per la ripetibilità ogni run registra il seed e i parametri in un file di configurazione JSON e utilizza logging strutturato per tracciare gli eventi salienti del training e delle valutazioni.
	
	\subsection{Script di sottomissione (Cluster GPU)}
	Lo script utilizzato per lanciare l'esecuzione di un notebook Jupyter sul cluster GPU (versione attuale usata per le sperimentazioni) è il seguente:
	
	\begin{verbatim}
		#!/bin/bash
		#SBATCH --job-name=PM_India
		#SBATCH --mail-type=ALL
		#SBATCH --mail-user=martin.tomassi@studio.unibo.it
		#SBATCH --time=120:00:00
		#SBATCH --nodes=1
		...
		source venv_pm/bin/activate
		jupyter nbconvert --to notebook 
				--execute Training_Evaluation_India.ipynb 
				--output Training_Evaluation_India_trained.ipynb
		jupyter nbconvert --to html Training_Evaluation_India_trained.ipynb
		deactivate
