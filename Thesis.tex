...
\begin{document}
	...
	Il dataset comprende le seguenti features:
	\begin{itemize}
	...
	L’analisi dei risultati ha messo in evidenza che, dal punto di vista delle metriche di classificazione, le differenze tra i modelli testati sono complessivamente ridotte: accuratezza, F1-score, AUC-ROC e AUC-PR si collocano su valori molto simili, con scarti percentuali spesso marginali. La vera distinzione tra i modelli emerge quindi principalmente sul piano della complessità architetturale. \\
	Per quanto riguarda le reti neurali, la \textbf{MLP} ha ottenuto un’accuratezza complessiva in test pari al $76\%$, con un F1-weighted di $0.7654$, un F1-macro di $0.7421$, un AUC-ROC pari a $0.938$ ed un AUC-PR di $0.800$. Nonostante valori complessivamente accettabili, la sensibilità sulle classi minoritarie si è dimostrata ridotta, limitando l’efficacia in scenari caratterizzati da distribuzioni sbilanciate. Con circa $7.750$ parametri, il modello si presenta come una soluzione leggera e facilmente scalabile, ma meno competitiva sul piano delle performance rispetto alle alternative. La \textbf{KAN} ha invece fornito risultati più equilibrati, raggiungendo un’accuratezza in test pari al $78\%$, con un F1-weighted di $0.7823$ e un F1-macro di $0.7608$, insieme a un AUC-ROC di $0.948$ e un AUC-PR di $0.840$. Questo incremento di precisione, unito al basso numero di parametri (circa $7.680$), conferma la capacità della KAN di garantire un buon compromesso tra accuratezza e parsimonia architetturale, rendendola una candidata ideale per scenari efficienza-aware. \\
	I modelli ensemble hanno mostrato un comportamento altrettanto interessante. La \textbf{Random Forest}, con circa $412$k nodi, ha raggiunto un’accuratezza dell’$80\%$, con un F1-weighted di $0.7997$ e un F1-macro di $0.7750$, supportati da un AUC-ROC di $0.952$ e un AUC-PR di $0.858$. Le sue prestazioni si sono dimostrate competitive e non molto distanti da quelle di MLP e KAN, ma la complessità computazionale rimane molto elevata, rendendolo meno adatto quando l’efficienza costituisce un vincolo operativo. \textbf{XGBoost}, infine, si è distinto come il modello leggermente più performante: l’accuratezza in test ha raggiunto l’$81\%$, con un F1-weighted di $0.8107$ e un F1-macro di $0.7901$, accompagnati da un AUC-ROC di $0.957$ e un AUC-PR di $0.875$. Tuttavia, tali miglioramenti rispetto a KAN e Random Forest sono marginali, mentre la complessità architetturale (circa $90$k parametri) risulta nettamente superiore rispetto alle reti leggere come MLP e KAN, pur restando inferiore alla Random Forest. \\
