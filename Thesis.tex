							...
							for i in range(len(model.width) - 1):
								if i < len(model.act_fun):
									...
									if hasattr(layer, 'coef') and layer.coef is not None:
										...
										temp_module.weight = torch.nn
											.Parameter(layer.coef.view(-1, 1))
										kan_modules_to_prune
											.append((temp_module, 'weight'))
			...
			for param_id, params in enumerate(
				...
				for fold_idx, (train_idx, val_idx) in enumerate(
					...
					if class_weights is not None:
						sample_weights = np.array([class_weights.get(label, 1.0) 
					...
					optimizer = torch.optim.Adam(
						model.parameters(), lr=train_params['lr'])
					criterion = nn.CrossEntropyLoss()
					stopper = EarlyStopper(
